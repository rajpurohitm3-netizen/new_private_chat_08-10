    ...
    try {
      ...
      if (storedPrivKey && storedPrivKey !== "undefined" && storedPrivKey !== "null") {
        try {
          ...
          // Use stored public key if available, otherwise regenerate from private if possible 
          // (Since we can't derive RSA public from private easily, we'll trust the profile's public key if local is missing)
          const pubKeyToUse = storedPubKey || profile?.public_key;

          if (profile?.public_key && pubKeyToUse !== profile.public_key) {
            console.warn("Local public key doesn't match DB, updating DB to match local");
            await supabase.from("profiles").update({ 
              public_key: pubKeyToUse,
          ...
          // Ensure both are in localStorage for next time
          if (pubKeyToUse && !storedPubKey) {
            localStorage.setItem(`pub_key_${session.user.id}`, pubKeyToUse);
          }
        } catch (e) {
